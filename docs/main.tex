...
\author{GILLARD Antonino Yann William}
\date{January 2026}
...
place $N$ queen pieces on the board such that they can not attack one another. In chess, a queen can attack pieces who are on the same row,
column or diagonal as itself.
\begin{wrapfigure}{r}{0.3\textwidth}
    \centering
    \includegraphics[width=0.25\textwidth]{ressources/n-queen.png}
    \caption{A solution to the N-queens problem for $N=8$}
\end{wrapfigure}
...
As such, we have the follwing constraints : 
\begin{align*}
...
It can also be interesting to note that the times for $N=1, 5 and 10$ are similar (approx. 300ms). 
We expect from a well defined model and a good solver that the case $N=1$ be over as fast as possible. 
...
\end{align*}
to the loss function. The diagaonal constraints
\begin{align*}
    &\forall d \in \llbracket 0, N-1 \rrbracket \quad 
...
is the amount of variable which blocks us. Indeed, Fixstars API is a service with different subscription tiers.
...
\section{Comparing different model}
...
here all variable must be different from one another, or 
\begin{align*}
...
However, here, for $N \leq 100$, we can observe fluctuations. Certains cases are more complex than others.
...
\begin{center}
    ...
    \captionof{figure}{Comparing execution times for both MiniZinc models}
\end{center}
...
whereas the interger variable model computes for up to $N=400$.

Difference between both models lie in the choice of variables. The choice to use integer variables already implements the row 
...
for each variables and thus a total cost of $\mathcal{O}\left(N^2\right)$. Furthermore instead of using $N^2$ varaiables, we only need to use $N$ varaiables.

This comparaison shows the importance of well defining one's model for optimization problems by trying to 
imply as much constraints as possible in the definition of the variables.
...
This is probably because the servers owned by Fixstars are tuned to perform these kind of computations instead of running the MiniZinc models on one's own computer.
...
of the integer variables. Although it would decrease the number of variables by a small margin with $N$ integer variables encoded each by $N-1$ boolean varaiables thus $N\left(N-1\right)$ boolean variables in total, 
...
As for the QUBO model, we can see that QUBO is way more efficient that the boolean varaible model in MiniZinc, as it is able to go up to $N=90$ at least. But it also shows the weakness of QUBO as even though it is faster for such computation, 
...
\section{Sources}
You can find the source code used for this report in the following \href{https://github.com/Gillan0/n_queens_optimization}{GitHub repository}.
